...
$(document).ready(function () {
    ...
    $(document).on('click', '#SkinChangerSettingModal', function () {
        ...
        $.ajax({
            ...
            success: function (data) { 
                ...
                $('.cnopick').html('<button id="SkinChangerSkins">' + selectel_skins + '</button>');
    ...
    $(document).on('click', '.sk-modal-weapon-sticker', function () {
        const id = $(this).attr('id');
        const id_active = $('.sk-modal-weapon-sticker.active').attr('id');
    
        const idNum = parseInt(id);
        const idActiveNum = parseInt(id_active);
    
        if (idNum === idActiveNum || (idActiveNum >= 1 && idActiveNum <= 4 && idNum !== 5)) {
            // error
        } else {
            $.ajax({
                type: 'post',
                url: location.href,
                data: { StickersKeychainHtml: true, id: idNum },
                dataType: 'json',
                global: false,
                success: function (data) {
    ...
    function renderPagination(totalStickers, stickersPerPage, currentPage) {
        ...
        if (totalPages > 1) {
            ...
            if (startPage > 1) {
                paginationContainer.append('<a class="button_pagination" data-page="1">«</a>');
            }
            if (currentPage > 1) {
                paginationContainer.append('<a class="button_pagination" data-page="' + (currentPage - 1) + '">‹</a>');
            }
            for (let i = startPage; i <= endPage; i++) {
                paginationContainer.append('<a class="button_pagination' + (i === currentPage ? ' active' : '') + '" data-page="' + i + '">' + i + '</a>');
            }
            if (endPage < totalPages) {
                paginationContainer.append('<a class="button_pagination" data-page="' + (currentPage + 1) + '">›</a>');
            }
            if (currentPage < totalPages) {
                paginationContainer.append('<a class="button_pagination" data-page="' + totalPages + '">»</a>');
            }
        }
    }    
    ...
    function closeSkinModal() { 
        $('.skin-modal, .skin-modal-overlay').removeClass('active-modal'); 
        $('.servers-modal, .servers-modal-overlay').addClass('no-active-modal'); 
        $('#SkinChangerSetting').remove(); 
        $('#SkinChangerNoSkin').remove(); 
        $('#skin-modal-js').html('<div class="block-loader-modal"><div class="loader-skins-modal"></div></div>');
        
        if (hasChanges && page_php == "weapons") {
            const idteam = localStorage.getItem('activeSide');
            const idserver = localStorage.getItem('activeServer');
            const idSortAll = localStorage.getItem('idSort');
